...
public class DbRecord
{
        ...
        throws FileNotFoundException, NullPointerException, AppException
    {
        ...
        try
        {
            ...
            byte[] Data_uname = null;
            byte[] Data_email = null;
            ...
            if( nFileSize < nLength )
                throw new AppException( "Bad file " + f.getPath() );
            nFileSize -= nLength;
            Data_uname = new byte[nLength];
            fs.read( Data_uname );
            m_UserName = utf8Decoder.decode( ByteBuffer.wrap( Data_uname ) ).toString();
            ...
            if( nFileSize < nLength )
                throw new AppException( "Bad file " + f.getPath() );
            nFileSize -= nLength;
            Data_email = new byte[nLength];
            fs.read( Data_email );
            m_email = utf8Decoder.decode( ByteBuffer.wrap( Data_email ) ).toString();
     ...
     */
    public boolean Save(String szFileName,String Email )
        throws NullPointerException, IllegalStateException, IOException
    {
        ...
        if( m_Template == null || m_UserName == null || m_UserName.length() == 0 || m_email.length()==0 )
        ...
        try
        {
            ...
            Data_uname = new byte[ bBuffer.limit()];
            bBuffer.get( Data_uname );
            fs.write( ((Data_uname.length >>> 8) & 0xFF) );
            fs.write( (Data_uname.length & 0xFF) );
            fs.write(Data_uname);
            
            byte[] Data_email = null;

           // Save email
            ByteBuffer bBuffer1 = utf8Encoder.encode( CharBuffer.wrap( m_email.toCharArray() ) );
            Data_email = new byte[ bBuffer1.limit()];
            bBuffer1.get( Data_email );
            fs.write( ((Data_email.length >>> 8) & 0xFF) );
            fs.write( (Data_email.length & 0xFF) );
            fs.write(Data_email);
